...
do_wget() {
  echo "trying wget..."
  wget --user-agent="User-Agent: mixlib-install/3.12.11" -O "$2" "$1" 2>$tmp_dir/stderr
  rc=$?
  # check for 404
...
do_curl() {
  echo "trying curl..."
  curl -A "User-Agent: mixlib-install/3.12.11" --retry 5 -sL -D $tmp_dir/stderr "$1" > "$2"
  rc=$?
  # check for 404
...
do_fetch() {
  echo "trying fetch..."
  fetch --user-agent="User-Agent: mixlib-install/3.12.11" -o "$2" "$1" 2>$tmp_dir/stderr
...
do_python() {
  echo "trying python..."
  python -c "import sys,urllib2; sys.stdout.write(urllib2.urlopen(urllib2.Request(sys.argv[1], headers={ 'User-Agent': 'mixlib-install/3.12.11' })).read())" "$1" > "$2" 2>$tmp_dir/stderr
  rc=$?
  # check for 404
...
# will 'inherit' the changes (chef_install.sh is not the only client of the omnitruck
# endpoint out there).
#
...
#
# NOTE: platform mangling in the chef_install.sh is DEPRECATED
#
# - chef_install.sh should be true to ohai and should not remap
